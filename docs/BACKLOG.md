...
- **QA note:** AC1 enum defined. `transition()` only blocks IN_TRANSIT/DELIVERED → CANCELLED; `DELIVERED → ASSIGNED` is still allowed (AC2). No timestamps/history (AC3).
...
- **QA note:** AC1 done via `cancel()`. AC2 needs a driver↔order link; AC3 needs a fee rule.
